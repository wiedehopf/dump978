...
namespace dump978 {
    SocketOutput::SocketOutput(asio::io_service &service,
                               tcp::socket &&socket)
        : service_(service),
          strand_(service),
    ...
    void SocketOutput::ReadAndDiscard()
    {
        ...
        socket_.async_read_some(asio::buffer(*buf),
                                strand_.wrap([this,self,buf] (const boost::system::error_code &ec, std::size_t len) {
                                        if (ec) {
                                            HandleError(ec);
                                        } else {
                                            ReadAndDiscard();
    ...
    void SocketOutput::Flush()
    {
        ...
        async_write(socket_, boost::asio::buffer(*writebuf),
                    strand_.wrap([this,self,writebuf] (const boost::system::error_code &ec, size_t len) {
                            flush_pending_ = false;
                            if (ec) {
                                HandleError(ec);
                                return;
    ...
    SocketListener::SocketListener(asio::io_service &service,
                               ...
                               [this,self] (const boost::system::error_code &ec) {
                                   if (!ec) {
                                       ...
                                       if (new_output) {
                                           ...
                                           new_output->SetCloseNotifier([this,self,handle] {
                                                   dispatch_.RemoveClient(handle);
                                               });
                                           new_output->Start();
                                       }
                                   } else {
