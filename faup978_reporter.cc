...
template <typename T>
static std::string value_map(T value, const std::map<T,std::string> &mappings, const std::string &default_value) {
...
template <typename T>
static std::function<void(std::ostream&)> simple_emit(const T& field, int precision = 0)
{
...
{    
    ...
    static std::map<AddressQualifier,std::string> idtype_map = {
        { AddressQualifier::ADSB_ICAO, "hexid" },
        { AddressQualifier::TISB_ICAO, "hexid" }
    };

    std::cout << value_map(aircraft.address_qualifier, idtype_map, "otherid")
              << '\t'
              << std::hex << std::uppercase << std::setfill('0') << std::setw(6) << aircraft.address
